...
export default function MostraAnnunci() {
  ...
  // Query senza pagina: usata per il fetch (cambio pagina non scatena nuovo fetch)
  const qs = (() => {
  ...
  // Lista id preferiti per evidenziare le card
  const [preferitiIds, setPreferitiIds] = useState([]);

  // Carica i preferiti quando si logga (o li svuota al logout)
  useEffect(() => {
    if (!utente) {
      setPreferitiIds([]);
      return;
